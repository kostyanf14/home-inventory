export const inventory = {
  totalItems: "Усього речей",
  medicines: "Ліки",
  equipment: "Обладнання",
  food: "Їжа",
  sites: "Місця",
  catalogReady: "Ваш каталог готовий до пошуку та сортування.",
  searchInventory: "Пошук інвентарю",
  show: "Показати:",
  allItems: "Усі речі",
  medicine: "Ліки",
  other: "Інше",
  foodFormPlaceholder: "напр. консерва, банка, сухе",
  yourInventoryIsClear: "Ваш інвентар порожній",
  inventoryEmptyDetail: "Додайте річ нижче, щоб створити надійну мапу ваших речей.",
  item: "Річ",
  location: "Локація",
  unassigned: "Не призначено",
  expires: "Термін придатності",
  warrantyEnds: "Гарантія до",
  quickAdd: "Швидке додавання",
  captureIt: "Додайте річ, поки вона під рукою.",
  itemName: "Назва речі",
  itemNamePlaceholder: "напр. Спрей для аптечки",
  barcode: "Штрихкод",
  barcodePlaceholder: "напр. 4006381333931",
  lookUpBarcode: "Знайти",
  scanBarcode: "Сканувати штрихкод",
  scanBarcodeAction: "Сканувати",
  scanBarcodeHint: "Наведіть камеру на штрихкод. Код заповниться автоматично.",
  barcodeScanFailed: "Не вдалося отримати доступ до камери. Перевірте дозволи та спробуйте ще раз.",
  barcodeScanRequiresHttps:
    "Сканування камерою потребує HTTPS. Введіть цифри штрихкоду нижче або відкрийте сайт через захищене з'єднання.",
  barcodeLookupHint: "Введіть 6–14 цифр і знайдіть товар. Спочатку перевіряється ваш каталог, потім Open Food Facts.",
  barcodeFound: "Знайдено в каталозі: {name}",
  barcodeFoundExternal: "Знайдено в Open Food Facts: {name}. Перевірте назву й додайте річ, щоб зберегти її.",
  barcodeNotFound: "Немає ні в каталозі, ні в Open Food Facts. Додайте річ, щоб зберегти штрихкод на наступний раз.",
  barcodeLookupInvalid: "Введіть 6–14 цифр, щоб знайти штрихкод.",
  barcodeLookingUp: "Пошук…",
  barcodeLookupFailed: "Не вдалося знайти цей штрихкод.",
  selectSite: "Оберіть місце",
  selectPlace: "Оберіть локацію",
  unit: "Одиниця",
  expirationDate: "Термін придатності",
  buyDate: "Дата придбання",
  warrantyExpirationDate: "Дата завершення гарантії",
  addToInventory: "Додати до інвентарю",
  createSiteAndPlace: "Створіть місце і локацію перед додаванням речі.",
  itemAdded: "Річ додано до інвентарю.",
  unableToAddItem: "Не вдалося додати річ.",
  deleteItem: "Видалити річ",
  deleteItemNamed: "Видалити {name}",
  confirmDeleteItem: 'Видалити "{name}" з інвентарю?',
  itemDeleted: "Річ видалено з інвентарю.",
  unableToDeleteItem: "Не вдалося видалити річ.",
  noMatchingItems: "Немає речей за вашим запитом",
  noMatchingItemsDetail: "Спробуйте іншу назву, локацію чи штрихкод або скиньте фільтр типу.",
  noPlacesInSite: "У цьому місці ще немає локацій",
} as const;
